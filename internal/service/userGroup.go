...
func (h *HTTPServer) CreateGroup(c *gin.Context) {
	...
	user.Id, _ = utils2.StringToUint(id)
	...
	if b {
		...
		utils2.Fail(c, CreateGroupFail, "create group err")
	}
}
...
func (h *HTTPServer) GetCreateGroupList(c *gin.Context) {
	...
	if b {
		...
		utils2.Fail(c, GetCreateGroupListFail, "get create group list err")
	}
}
...
func (h *HTTPServer) GetAllGroupList(c *gin.Context) {
	...
	if b {
		...
		utils2.Fail(c, GetAllGroupListFail, "get group list err")
	}
...
func (h *HTTPServer) AddGroup(c *gin.Context) {
	...
	groupID, _ := utils2.StringToUint(groupIDString)
	...
	if result != nil {
		...
		utils2.Fail(c, AddGroupFail, "add group err")
	}
...
func (h *HTTPServer) DisposeAddGroup(c *gin.Context) {
	...
	groupID, _ := utils2.StringToUint(groupIDString)
	state, _ := utils2.StringToUint8(stateString)

	user.Id, _ = utils2.StringToUint(c.GetHeader("id"))

	result, b := user.DisposeAddGroup(groupID, state)
	if b {
		utils2.Success(c, SUCCESS, result)
	} else {
		utils2.Fail(c, DisposeAddGroupFail, result)
	}
}
