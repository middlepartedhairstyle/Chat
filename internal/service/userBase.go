	...
	if user.Code == code {
		...
		if b, _ := user.EmailIsUser(); b {
			...
			if user.CreateUser() {
				...
				utils2.Fail(c, CreateUserFail, gin.H{
					"err_msg": "用户创建失败",
				})
			}
		}
		...
		utils2.Fail(c, VerifyCodeFail, gin.H{
			"err_msg": "验证码错误",
		})
	}
	//删除验证码
...
func (h *HTTPServer) PassWordLogin(c *gin.Context) {
	...
	if b, _ := user.EmailIsUser(); b {
		if user.CheckPassword() {
			...
			utils2.Fail(c, WrongPassword, gin.H{
				"err_msg": "密码错误",
			})
		}
	} else {
		utils2.Fail(c, NotFoundUser, gin.H{
			"err_msg": "用户不存在",
		})
	}
}
...
func (h *HTTPServer) VerifyCode(c *gin.Context) {
	...
	if user.VerifyCode == user.Code {
		...
		utils2.Fail(c, VerifyFail, gin.H{
			"err_msg": "验证错误",
		})
	}
}
...
func (h *HTTPServer) ChangeUserName(c *gin.Context) {
	...
	if !err {
		utils2.Fail(c, QueryNameError, gin.H{
			"err_msg": "query name error",
		})
		return
	}
	user.Id, _ = utils2.StringToUint(ids)
...
func (h *HTTPServer) ChangeUserEmail(c *gin.Context) {
	...
	code.Id, _ = utils2.StringToUint(ids)
