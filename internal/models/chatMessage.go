...
import (
	"context"
	"fmt"
	Kafka2 "github.com/middlepartedhairstyle/HiWe/internal/kafka"
...
type Information interface {
	MessageDispose(producers map[string]*Kafka2.Producer, fromID uint, message []byte, args ...map[string]uint)
	//SetConsumerID() uint
...
// MessageDispose 消息处理
func (userMessage *UserChatMessage) MessageDispose(producers map[string]*Kafka2.Producer, fromID uint, message []byte, args ...map[string]uint) {
...
func (userMessage *UserChatMessage) IsFriendMessage(producers map[string]*Kafka2.Producer, fromID uint, message []byte, infoVerify map[string]uint) {
	//消息正确性验证
	if userMessage.FromID == fromID && userMessage.JudgeFriend(infoVerify) {
		var producer *Kafka2.Producer
		...
		if producers[userMessage.SetTopic(toFriendID)] != nil {
			...
			producer = Kafka2.NewProducer(Kafka2.SetProducerTopic(topic))
...
func (userMessage *UserChatMessage) IsGroupMessage(producers map[string]*Kafka2.Producer, fromID uint, message []byte, infoVerify map[string]uint) {
	//消息来源正确性验证
...
func (userMessage *UserChatMessage) FriendMessageTypeDispose() bool {
	...
	case MessageTypeText:
		friendMessage := tables2.NewFriendMessage(userMessage.FromID, userMessage.ToID, userMessage.MessageType, &(userMessage.Message))
...
func (userMessage *UserChatMessage) GroupMessageTypeDispose() bool {
	...
	//消息类型为文本
	case MessageTypeText:
		groupMessage := tables2.NewGroupMessage(userMessage.FromID, userMessage.ToID, userMessage.MessageType, &(userMessage.Message))
...
func (userMessage *UserChatMessage) JudgeFriend(infoVerify map[string]uint) bool {
	...
	if infoVerify[ChatWithFriend+strconv.Itoa(int(userMessage.ToID))] != 0 {
		...
		friend := tables2.NewFriend(tables2.SetFriendID(userMessage.ToID), tables2.SetUserOneID(userMessage.FromID))
...
func (userMessage *UserChatMessage) JudgeGroupUser(infoVerify map[string]uint) bool {
	if infoVerify[ChatWithGroup+strconv.Itoa(int(userMessage.ToID))] != 0 {
		...
		groupUser := tables2.NewGroupUser(tables2.SetUserID(userMessage.FromID), tables2.SetGroupID(userMessage.ToID))
...
func (userMessage *UserChatMessage) GetFriendMessage(id uint, ws *WebSocketClient) {
	...
	consumer := Kafka2.NewConsumer(Kafka2.SetConsumerTopic(topic), Kafka2.SetConsumerGroupID(ChatWithFriend+strconv.Itoa(int(id))))
...
func (userMessage *UserChatMessage) GetGroupMessage(id uint, ws *WebSocketClient) {
	...
	groupUser := tables2.NewGroupUser(tables2.SetUserID(id))
	...
	for _, topic := range topics {
		go func(topic string, groupID []uint) {
			consumer := Kafka2.NewConsumer(Kafka2.SetConsumerTopic(topic), Kafka2.SetConsumerGroupID(topic+ChatGroupUser+strconv.Itoa(int(id)))) //消费者的id，gtp1gu1,gtp2gu1(后期更改为GroupUserID)
			defer func(consumer *kafka.Reader) {
				err := consumer.Close()
...
func (userMessage *UserChatMessage) ChangeGroupMessage(groupID uint, id uint, ws *WebSocketClient) {
	...
	go func(topic string, groupID uint) {
		consumer := Kafka2.NewConsumer(Kafka2.SetConsumerTopic(topic), Kafka2.SetConsumerGroupID(topic+ChatGroupUser+strconv.Itoa(int(id)))) //gtp1gu1,gtp2gu1
		defer func(consumer *kafka.Reader) {
			err := consumer.Close()
			if err != nil {
