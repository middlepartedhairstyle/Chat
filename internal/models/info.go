...
func (info *Info) WriteKafka(disposeInfo DisposeInfo, args ...interface{}) error {
	...
	if len(args) >= 2 {
		...
		defer func(producer *Kafka2.Producer) {
...
func (info *Info) ReadKafka(userID uint, ws *WebSocketClient) {
	...
	consumer := Kafka2.NewConsumer(Kafka2.SetConsumerTopic(topic), Kafka2.SetConsumerGroupID(UserMessageBaseGroup+strconv.Itoa(int(userID))))
	defer func(consumer *kafka.Reader, producer *Kafka2.Producer) {
		_ = consumer.Close()
