...
import (
	...
	redis2 "github.com/middlepartedhairstyle/HiWe/internal/redis"
...
func (u *UserVerify) CheckCode(key string) bool {
	u.VerifyCode, _ = redis2.Rdb.Get(context.Background(), key).Result()
...
func (user *UserBaseInfo) CreateUser() bool {
	...
	u.Sale = utils2.RandString()                                               //用户密码密钥
	u.Password = utils2.MakePasswordSha256(user.Password, u.Sale)              //用户密码
	u.Token = utils2.MakeToken(user.Email, user.Password, utils2.RandString()) //用户token
	err := u.Create()
	//跟新redis token
	redis2.UpdateToken(u.ID, u.Token)
	if err != nil {
		return false
	} else {
		return true
...
func (user *UserBaseInfo) DeleteUser() bool {
	var u tables2.UserBaseInfo
	u.ID = user.Id
	return u.DeleteUser() && redis2.DeleteToken(u.ID)
}
...
func (user *UserBaseInfo) CheckPassword() bool {
	...
	//验证密码准确性
	if b && utils2.CheckPasswordSha256(user.Password, u.Sale, u.Password) {
		return true
	}
	return false
}
...
func (user *UserBaseInfo) UpdateToken() bool {
	var u tables2.UserBaseInfo
	u.Email = user.Email
	u.Token = utils2.MakeToken(user.Email, user.Password, utils2.RandString())
	//数据库token更新
	if u.UpdateToken() && u.FindId() {
		//redis token更新
		return redis2.UpdateToken(u.ID, u.Token)
	}
	return false
}
...
func (user *UserBaseInfo) CheckToken() bool {
	var u tables2.UserBaseInfo
	if redis2.CheckToken(user.Id, user.Token) {
		return true
	} else {
		u.Token = user.Token
		u.ID = user.Id
...
func (user *UserBaseInfo) ChangePassword() bool {
	var u tables2.UserBaseInfo
	u.ID = user.Id
	u.Sale = utils2.RandString()
	u.Password = utils2.MakePasswordSha256(user.Password, u.Sale)
	u.UseIDFindEmail()
	fmt.Println(u.Email)
	u.Token = utils2.MakeToken(user.Email, user.Password, utils2.RandString())
	if u.ChangePassword() {
...
func (user *UserBaseInfo) GetFriendList() ([]tables2.Friends, bool) {
	var f tables2.Friends
	return f.GetFriendList(user.Id)
}

// GetRequestFriendList 获取好友请求添加列表
func (user *UserBaseInfo) GetRequestFriendList() ([]tables2.RequestFriend, bool) {
	var f tables2.RequestFriend
...
func (user *UserBaseInfo) DisposeAddFriend(f tables2.Friends, requestId uint, state uint8) (bool, uint8) {
	var r tables2.RequestFriend
	//好友添加请求id
	r.ID = requestId
	...
	if r.ID == r.GetID() {
		//获取好友请求表内的状态
		r.GetState()
		if state == tables2.Agree && r.State == tables2.Await {
			//判断是否为好友
			if !f.IsFriend() {
				//添加好友
				if f.AddFriend() {
			...
			//0表示已为好友
			return false, 0
		} else if state == tables2.Refuse && r.State == tables2.Await {
			r.SetState(state)
...
func (user *UserBaseInfo) ChangeFriendNote(friendID uint, note string) bool {
	var friend = tables2.NewFriend(tables2.SetFriendID(friendID))
...
// CreateGroup 新建群
func (user *UserBaseInfo) CreateGroup(groupName string) (*tables2.GroupNum, bool) {
	groupNum := tables2.NewGroupNum(tables2.SetGroupLeaderID(user.Id), tables2.SetGroupName(groupName))
...
func (user *UserBaseInfo) FindAllCreateGroup() ([]tables2.GroupNum, bool) {
	groupNum := tables2.NewGroupNum(tables2.SetGroupLeaderID(user.Id))
	return groupNum.FindAllCreateGroup()
}

// FindAllGroup 寻找用户加入的所有群聊
func (user *UserBaseInfo) FindAllGroup() ([]tables2.GroupUser, bool) {
	groupUser := tables2.NewGroupUser(tables2.SetUserID(user.Id))
	return groupUser.FindAllGroup()
}

// FindGroup 使用群id或群名寻找群
func (user *UserBaseInfo) FindGroup(groupInfo string) []tables2.GroupNum {
	info, err := utils2.StringToUint(groupInfo)
	switch err {
	case nil:
		group := tables2.NewGroupNum(tables2.SetGroupNumID(info))
		return group.UseGroupIDFind()
	default:
		group := tables2.NewGroupNum(tables2.SetGroupName(groupInfo))
...
func (user *UserBaseInfo) AddGroup(groupID uint) (interface{}, uint8) {
	group := tables2.NewGroupNum(tables2.SetGroupNumID(groupID))
	switch group.IsVerify() {
	//不需要验证
	case 0:
		groupUser := tables2.NewGroupUser(tables2.SetUserID(user.Id), tables2.SetGroupID(groupID))
	...
	case 1:
		...
		groupRequest := tables2.NewRequestAddGroup(tables2.SetFromRequestID(user.Id), tables2.SetToRequestID(group.GroupLeaderID), tables2.SetAddGroupID(group.ID))
...
func (user *UserBaseInfo) DisposeAddGroup(requestID uint, state uint8) (uint8, bool) {
	request := tables2.NewRequestAddGroup(tables2.SetRequestAddGroupID(requestID), tables2.SetState(state), tables2.SetToRequestID(user.Id))
	if request.ChickToUser() {
		switch request.State {
		case 1:
			if request.ChangeState() {
				//创建群用户，将被确认者加入群中
				groupUser := tables2.NewGroupUser(tables2.SetUserID(request.FromRequestID), tables2.SetGroupID(request.AddGroupID))
				if groupUser.CreateGroupUser() {
