...
func (group *GroupNum) CreateGroup() bool {
	//检查是否超过最大群数限制
	var count int64
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("group_leader_id = ?", group.GroupLeaderID).Count(&count).Error
	if count < maxGroupNum {
		//若小于最大群数则创建群
		err = mySQL2.DB.Table(mySQL2.GroupNumT).Create(group).Error
		if err != nil {
			return false
		}
		//创建成功将信息同时存入group_user_tables
		err = mySQL2.DB.Table(mySQL2.GroupNumT).Where("group_leader_id = ?", group.GroupLeaderID).First(&group).Error
...
func (group *GroupNum) FindAllCreateGroup() ([]GroupNum, bool) {
	var groups []GroupNum
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("group_leader_id = ?", group.GroupLeaderID).Find(&groups).Error
	if err != nil {
		return nil, false
	}
	return groups, true
...
func (group *GroupNum) UseGroupIDFind() []GroupNum {
	var groups []GroupNum
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("id = ? and visible = ?", group.ID, true).Find(&groups).Error
	if err != nil {
		return nil
	}
	return groups
...
func (group *GroupNum) UseGroupNameFind() []GroupNum {
	var groups []GroupNum
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("group_name = ? and visible = ?", group.GroupName, true).Find(&groups).Error
	if err != nil {
		return nil
	}
	return groups
...
func (group *GroupNum) IsVerify() uint8 {
	var verify uint8
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("id = ?", group.ID).Select("verify").Scan(&verify).Error
	if err != nil {
		return 127
	}
	return verify
...
func (group *GroupNum) GetGroupLeaderID() {
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("id = ?", group.ID).Select("group_leader_id").Scan(&(group.GroupLeaderID)).Error
	if err != nil {
		return
	}
}
