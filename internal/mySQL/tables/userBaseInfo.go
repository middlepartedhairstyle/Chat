...
func (user *UserBaseInfo) UseEmailSelect() (bool, error) {
	var count int64
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("email=?", user.Email).Count(&count).Error
	//错误处理,不为空返回错误
	if err != nil {
		return false, err
	}
	...
	if count <= 0 {
		...
		//有该邮箱就将用户数据写入该结构体中
		err = mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("email=?", user.Email).Select("*").Scan(user).Error
		//如果错误返回err
		if err != nil {
			return true, err
		}
...
func (user *UserBaseInfo) UseUserIDSelectPassword() bool {
	if user.CheckUserID() {
		err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Select("password").Scan(&user.Password).Error
		if err != nil {
			return false
		}
		return true
...
func (user *UserBaseInfo) Create() error {
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Create(user).Error
	if err != nil {
		return err
	}
	return nil
...
func (user *UserBaseInfo) UpdateToken() bool {
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("email=?", user.Email).Update("token", user.Token).Error
	if err != nil {
		return false
	}
	return true
...
func (user *UserBaseInfo) CheckToken() bool {
	var token string
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Select("token").Scan(&token).Error
	if err != nil {
		return false
	}
	if token == user.Token {
...
func (user *UserBaseInfo) FindId() bool {
	var id uint
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("email=?", user.Email).Select("id").Scan(&id).Error
	if err != nil {
		return false
	}
	if id != 0 {
...
func (user *UserBaseInfo) UseIDFindEmail() bool {
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Select("email").Find(&user.Email).Error
	if err != nil {
		return false
	} else {
		return true
...
func (user *UserBaseInfo) CheckUserID() bool {
	var count int64
	err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Count(&count).Error
	if err != nil {
		return false
	}
	if count > 0 {
...
func (user *UserBaseInfo) ChangeBaseUserInfo(columnName string, value string) bool {
	if user.CheckUserID() {
		err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Update(columnName, value).Error
		if err != nil {
			return false
		} else {
			return true
...
func (user *UserBaseInfo) ChangePassword() bool {
	if user.CheckUserID() {
		err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Updates(map[string]interface{}{
...
func (user *UserBaseInfo) DeleteUser() bool {
	if user.CheckUserID() {
		err := mySQL2.DB.Table(mySQL2.UserBaseInfoT).Where("id=?", user.ID).Delete(user).Error
		if err != nil {
			return false
		}
		return true
