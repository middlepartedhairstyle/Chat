...
func (groupMessage *GroupMessage) CreateGroupMessage() bool {
	var count int64
	err := mySQL2.DB.Table(mySQL2.GroupNumT).Where("id=?", groupMessage.GroupID).Count(&count).Error
	if err != nil {
		return false
	}
	if count > 0 {
		err = mySQL2.DB.Table(mySQL2.GroupMessageT).Create(groupMessage).Error
		if err != nil {
			return false
		}
		return true
