...
func (friendMessage *FriendMessage) CreateFriendMessage() bool {
	var count int64
	err := mySQL2.DB.Table(mySQL2.FriendT).Where("id=?", friendMessage.FriendID).Count(&count).Error
	if err != nil {
		return false
	}
	if count > 0 {
		err = mySQL2.DB.Table(mySQL2.FriendMessageT).Create(friendMessage).Error
		if err != nil {
			return false
		}
		return true
