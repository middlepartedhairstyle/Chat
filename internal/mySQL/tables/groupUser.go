...
func (groupUser *GroupUser) CreateGroupUser() bool {
	var count1 int64
	var count2 int64
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("group_id=? and user_id=?", groupUser.GroupID, groupUser.UserID).Count(&count1).Error
	if err != nil {
		return false
	}
	err = mySQL2.DB.Table(mySQL2.GroupNumT).Where("id=?", groupUser.GroupID).Count(&count2).Error
	if err != nil {
		return false
	}
	if count1 <= 0 && count2 > 0 {
		err = mySQL2.DB.Table(mySQL2.GroupUserT).Create(groupUser).Error
		if err != nil {
			return false
		}
		return true
...
func (groupUser *GroupUser) FindAllGroup() ([]GroupUser, bool) {
	...
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("user_id = ?", groupUser.UserID).Find(&groupUsers).Error
	if err != nil {
...
func (groupUser *GroupUser) FindAllGroupID() []uint {
	var groupIDs []uint
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("user_id = ?", groupUser.UserID).Select("group_id").Scan(&groupIDs).Error
	if err != nil {
		return nil
	}
	return groupIDs
...
func (groupUser *GroupUser) FindGroupUserID() []uint {
	...
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("user_id = ?", groupUser.UserID).Select("id").Scan(&groupUserIDs).Error
	if err != nil {
		return nil
	}
	return groupUserIDs
...
func (groupUser *GroupUser) FindAllGroupUser() []GroupUserInfo {
	...
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("group_id = ?", groupUser.GroupID).Select("id,group_id,user_id,level,relationship").Find(&groupUsers).Error
	if err != nil {
		return nil
	}
	return groupUsers
...
func (groupUser *GroupUser) IsGroupUserGetID() bool {
	var count int64
	err := mySQL2.DB.Table(mySQL2.GroupUserT).Where("group_id=? and user_id=?", groupUser.GroupID, groupUser.UserID).Count(&count).Error
	if err != nil {
		return false
	}
	if count > 0 {
		err = mySQL2.DB.Table(mySQL2.GroupUserT).Where("group_id=? and user_id=?", groupUser.GroupID, groupUser.UserID).Select("id").Scan(&groupUser.ID).Error
		if err != nil {
			return false
		}
		return true
