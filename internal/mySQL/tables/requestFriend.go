...
func (request *RequestFriend) InsertInto() bool {
	...
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Create(request).Error
	if err != nil {
		return false
	}
	return true
...
func (request *RequestFriend) HaveRequest() bool {
	var count int64
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("from_request_id=? AND to_request_id=?", request.FromRequestID, request.ToRequestID).Count(&count).Error
	if err != nil {
		return false
	}
	return count > 0
...
func (request *RequestFriend) GetID() uint {
	var id uint
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("from_request_id=? AND to_request_id=?", request.FromRequestID, request.ToRequestID).Select("id").Find(&id).Error
	if err != nil {
		return 0
	}
	return id
...
func (request *RequestFriend) GetState() {
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("id", request.ID).Select("state").Find(&(*request).State).Error
	if err != nil {
		return
	}
}
...
func (request *RequestFriend) GetAllRequest() ([]RequestFriend, bool) {
	...
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("to_request_id=? OR from_request_id=?", request.ToRequestID, request.ToRequestID).Find(&requestList).Error
	if err != nil {
		return nil, false
	} else {
...
func (request *RequestFriend) SetState(state uint8) {
	err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("id", request.ID).Update("state", state).Error
	if err != nil {
		return
	}
}
...
func (request *RequestFriend) RemoveRequest() bool {
	if err := mySQL2.DB.Table(mySQL2.RequestAddFriendT).Where("id = ?", request.ID).Delete(&mySQL2.RequestAddFriendTable{}); err != nil {
